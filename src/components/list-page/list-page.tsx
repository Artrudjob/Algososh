...
export const ListPage: React.FC = () => {
  ...
  const [linkedList, setLinkedList] = React.useState<LinkedList<string>>(randomArr);
  const [elements, setElements] = React.useState<JSX.Element[]>([]);
  const [disabledBtn, setDisabledBtn] = React.useState<string>("disableOff");
  const [loaderAddHead, setLoaderAddHead] = React.useState<boolean>(false);
  const [loaderAddTail, setLoaderAddTail] = React.useState<boolean>(false);
  const [loaderDelHead, setLoaderDelHead] = React.useState<boolean>(false);
  const [loaderDelTail, setLoaderDelTail] = React.useState<boolean>(false);
  const [loaderAddByIndex, setLoaderAddByIndex] = React.useState<boolean>(false);
  const [loaderDelByIndex, setLoaderDelByIndex] = React.useState<boolean>(false);
  ...
  async function deleteTail() {
    ...
    setDisabledBtn("disabledOn");
    const arraySize = linkedList.toArray().length;
    const circleTail = <Circle letter={resultNodes[arraySize - 1].letter} state={ElementStates.Changing} isSmall={true} />
    if (arraySize === 1) {
      ...
      updateResultNodes(linkedList.toArray());updateCircleElement(resultNodes);
    } else {
      resultNodes[arraySize - 1].letter = "";
      resultNodes[arraySize - 1].tail = circleTail;
    ...
    <SolutionLayout title="Связный список">
      <div className={styles.list}>
        ...
        <Button type={"button"} text={"Удалить по индексу"} linkedList={"big"}
                ...
                disabled={linkedList.toArray().length === 0 || indexValue.length === 0 || disabledBtn === "disabledOn"}
